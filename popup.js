(function() {
    ...
    function initPopup() {
        ...
        // ПРОВЕРКА: Если попап уже есть, не создаем дубликат
        if (document.getElementById('my-custom-popup-overlay')) return;

        // СОЗДАНИЕ ЭЛЕМЕНТОВ
        const overlay = document.createElement('div');
        overlay.id = 'my-custom-popup-overlay'; // Добавили ID для проверки
        ...
        // ВАЖНО: Добавляем проверку наличия body
        if (document.body) {
    ...
    // === БЛОК ОЖИДАНИЯ ЗАГРУЗКИ ===
    // Если страница еще грузится, ждем события DOMContentLoaded
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initPopup);
    } else {
        // Если уже загрузилась — запускаем сразу
        initPopup();
    }
})();
